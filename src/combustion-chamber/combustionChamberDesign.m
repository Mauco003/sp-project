...
% combustionChamber - Design the combustion chamber and nozzle liner thicknesses based on the performance and thermal requirements.
%
% SYNTAX:
%  sol = combustionChamberDesign(input, performanceNom, grain, nozzle, ...
%     propellant, casing, liner, constants)
%
% INPUT:
%  input            - struct containing design input parameters
%  performanceNom   - struct containing nominal performance parameters
%  grain            - struct containing grain geometry and properties
%  nozzle           - struct containing nozzle geometry and properties
%  propellant       - struct containing propellant properties
%  casing           - struct containing casing properties
%  liner            - struct containing liner properties
%  constants        - struct containing physical constants
%
% OUTPUT:
%  sol              - struct containing optimal design solution with fields:
%                       .J              - objective function value (cost + mass)
%                       .tLinerCC       - optimal liner thickness in combustion chamber [m]
%                       .tCasing        - optimal casing thickness [m]
%                       .mass           - total mass of the combustion chamber and liner [kg]
%                       .cost           - total cost of the combustion chamber and liner [USD]
%                       .results        - struct containing results from the thermal model for the optimal design
%                       .tLinerNozzle   - optimal liner thickness in nozzle [m]
